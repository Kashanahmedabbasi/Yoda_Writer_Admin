...
const CategoryPageTables = () => {
    ...
    const columns = [
                ...
                <span
                    className={`${
                        params.value === "Deactive"
                            ...
                            : params.value === "Active"
                            ? "bg-green-100 border-green-100 text-green-500"
                            : ""
        ...
        }, //For Bg color changing when pending and active},
        {
            field: "action",
            headerName: "Action",
            flex: iswidth ? undefined : 1,
            ...
            renderCell: (params) => (
                <div className="flex space-x-2">
                    <RiEditBoxFill
                        size={20}
                        onClick={() => toggleEditModal(params.row.id)}
    ...
    return (
        <Container
            fluid
            className="flex flex-col items-center justify-center  w-full bg-[#F8F8F8] px-4 py-1 border rounded-lg overflow-hidden"
        >
            {/* CateGory Component */}
            <div className="flex flex-col items-center justify-between w-full py-4 border-b sm:flex-row gap-y-2">
                <h2 className="text-xl font-semibold">Category List</h2>
                <button
                    onClick={toggleAddModal}
                    className="px-4 py-3 font-semibold text-white bg-blue-500 rounded-lg"
                >
                    Add Category
                </button>
            </div>
            ...
            <div className="flex flex-col w-full py-2 my-3 overflow-x-auto bg-white border gap-y-2">
                    ...
                    <div className="text-sm flex flex-col lg:flex-row gap-y-2 lg:gap-y-0 items-center justify-between text-[#83878a] px-4 py-2 ">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                className="border py-3 px-4 focus:outline-blue-400 w-[100%] sm:w-auto"
                                type="text"
                            />
                        </div>
                    </div>
                ...
                <div className="px-4 text-sm text-[#83878a] flex justify-between items-center pb-2">
                    <h6>{`Showing ${startIndex + 1} to ${
                        endIndex > categoryData.length
                            ? categoryData.length
                            : endIndex
                    } of ${categoryData.length} entries`}</h6>
            ...
            <DeleteModalCategory
                ...
                isToggle={deleteModal}
                setIsToggle={setDeleteModal}
            />
            <EditModalCategory
                button={"Update"}
                heading={"Edit"}
                isToggle={editModal}
                setIsToggle={setEditModal}
                handlerFunction={handleEdit}
                name={editedName}
                status={editedStatus}
                setName={setEditedName}
                setImageFunction={handleImageChange}
            ...
            <EditModalCategory
                ...
                heading={"Add"}
                isToggle={addModal}
                setIsToggle={setAddModal}
                handlerFunction={handleAdd}
                name={addedName}
                status={addedStatus}
                setName={setAddedName}
                setImageFunction={handleImageAdd}
                setStatus={setAddedStatus}
...
export default CategoryPageTables;
